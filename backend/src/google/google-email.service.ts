...
export class GoogleEmailService implements OnModuleInit {
  ...
  async sendCandidateNotification(candidate: CandidateData, driveLink: string): Promise<boolean> {
    ...
    const nombre = candidate.datosFormulario['Nombre completo'] || 'Sin nombre';
    const cedula = candidate.datosFormulario['Cédula'] || 'Sin cédula';
    const telefono = candidate.datosFormulario['Teléfono'] || '-';
    const email = candidate.datosFormulario['Email'] || 'Sin email';

    const subject = `Nueva postulación: ${candidate.puesto} — ${nombre}`;
    const body = [
    ...
    try {
      ...
      this.logger.log(`Notification email sent to ${this.rrhhEmail} for candidate ${nombre}`);
      return true;
    } catch (error) {
